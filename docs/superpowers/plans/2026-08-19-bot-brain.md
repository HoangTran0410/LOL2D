...
- Produces:
  ...
  - On `BotBrain`: `rng: () => number`, `withinManaBudget(spell, mask): boolean`, `scoreSpell(spell, slotIndex, mask, target, view): number`, `chooseSpell(target, view): SpellChoice | null`, `chooseGhostSpell(entry, nowMs): SpellChoice | null`
