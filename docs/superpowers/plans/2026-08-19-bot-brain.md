...
describe('rolesOf()', () => {
  ...
  it('infers once per class, not once per instance', () => {
    ...
    rolesOf(spellLike(Counted, counting), 1);
...
describe('BotBrain perception', () => {
  ...
  it('never sees a stealthed enemy, however high the difficulty', () => {
    ...
    for (const tier of ['easy', 'normal', 'hard'] as const) {
      ...
      vi.spyOn(enemy, 'isStealthed', 'get').mockReturnValue(true);
      indexObjects(game, [bot, enemy]);
      expect(new BotBrain(bot).canPerceive(enemy)).toBe(false);
    }
  });
...
describe('posture', () => {
  ...
  it('engages a focus target an ally is fighting', () => {
    ...
    const mate = spawnEnemy(game, 600, 0); // position only; team set below
...
const makeSpell = (
  ...
  class Stub {
    ...
    castSpec = { targeting: 'DIRECTION' as const, range: over.range ?? 500 };
  }
...
describe('spell choice', () => {
  ...
  it('turns into a different bot when the noise is turned up', () => {
    ...
    brain.rng = () => 0;
    expect(brain.chooseSpell(enemy, view())?.slotIndex).toBe(1);
    // easy noise is 0.9. Buff base 5 * 1.9 = 9.5; Damage base 10 * 1.0 = 10 when
    // the first roll is 0 — so a roll that favours the second spell flips it.
    let call = 0;
...
```ts
  ...
  private maybeCast(
    ...
    if (posture === 'SEARCH') {
      const entry = this.rememberedTarget(view, nowMs);
      const ghost = entry ? this.chooseGhostSpell(entry, nowMs) : null;
      if (ghost && entry) this.cast(ghost, this.searchPoint(entry, nowMs), nowMs);
      return;
    }
...
Add the field `private _nowMs = 0;` and `readonly brain = new BotBrain(this);`.
