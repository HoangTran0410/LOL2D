 ...
 * them, and it does not need them: `spells/index.ts` is still the barrel the
 * generator reads to build `spellModules.ts`, so importing it here registers
 * exactly the same set the browser would end up with.
 *
...
export function loadEverySpellForTests(): void {
  ...
  for (const [id, spellClass] of Object.entries(AllSpells)) {
