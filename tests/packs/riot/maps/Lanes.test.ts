...
import { LANES, Lane, getLaneWaypoints, type LaneWaypoint } from '../../../../src/game/lanes';
import mapData from '../../../../packs/riot/maps/summoner_map.json';
import {
  minionMusterPoint,
  summonersRiftGeometry,
  SR_LANE_WAYPOINTS,
} from '../../../../packs/riot/maps/summonersRiftGeometry';
 ...
 * calling `setActiveLanes` itself: `tests/setup.ts` installs Summoner's
 * Rift's own lanes for every test file's environment by default (see that
 * file's own doc comment), so this file's whole job is checking that
 * installed content is correct — not installing it.
 */
