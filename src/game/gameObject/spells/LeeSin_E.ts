...
export default class LeeSin_E extends Spell {
  ...
  onSpellCast() {
    ...
    enemies.forEach((enemy: any) => {
      const slowBuff = new Slow(2000, this.owner, enemy);
    ...
    const obj = new LeeSin_E_Object(this.owner);
...
export class LeeSin_E_Object extends SpellObject {
  ...
  draw() {
    ...
    const alpha = map(this.age, 0, this.lifeTime, 180, 10);
    ...
    fill(255, 190, 30, alpha);
    const sizeIncrease = map(this.age, 0, this.lifeTime, 0, 50);
