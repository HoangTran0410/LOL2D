    ...
    dashBuff.onReachedDestination = () => {
      ...
      for (const enemy of enemiesInRange) {
        ...
        buff.draw = () => {
          ...
          const size = enemy.stats.size.value;
          stroke(random(200, 255));
          for (let i = 0; i < 1; i++) {
            const x1 = x + random(-size, size);
            const y1 = y + random(-size, size);
            const x2 = x + random(-size, size);
            const y2 = y + random(-size, size);
            line(x1, y1, x2, y2);
          }
      ...
      const speedup = new Speedup(3000, this.owner, this.owner);
      speedup.percent = 0.4;
...
export class Yasuo_R_Object extends SpellObject {
  ...
  draw() {
    push();
    const alpha = map(this.age, 0, this.lifeTime, 100, 50);
    stroke(255, alpha);
